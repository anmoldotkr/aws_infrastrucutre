...
ENV_FILE_PATH="/tmp/.env-ai.runtime"

# Target directory /home/ubuntu/app
COMPOSE_DIR="/home/ubuntu/app"
COMPOSE_FILE_PATH="${COMPOSE_DIR}/ai-docker-compose.yml"
...
echo 'Generating ai-docker-compose.yml on-the-fly at /home/ubuntu/app...'
...
services:
  xyz-ai:
    container_name: '$environment-$product-xyz-ai'
    image: '$IMAGE_URI:$SHORT_SHA'
    restart: always
    ports:
      - '8001:8001'
    environment:
      - REDIS_HOST=$environment-$product-redis
    env_file:
      - '$ENV_FILE_PATH'
    networks:
      - app_network
